...
    return <p className="text-xl text-gray-600 text-center p-4">You have not been assigned any classes yet.</p>;
  }


  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-4">
      <h1 className="text-3xl sm:text-4xl font-bold text-center text-indigo-800 mb-8">My Assigned Students</h1>
      
      
      <div className="mb-6 p-4 bg-white rounded-lg shadow-xl border border-gray-200">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-3">
          <div className="relative w-full sm:w-1/2">
            <input
              type="text"
...
        
        {showAdvancedFilters && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4 p-4 bg-gray-50 rounded-md shadow-inner">
            
...
                <div>
                    <label htmlFor="filterClassNumber" className="block text-sm font-medium text-gray-700">{selectedAcademicType.name} Grade</label>
                    <select
...
                <>
                    <div>
                        <label htmlFor="filterDegreeName" className="block text-sm font-medium text-gray-700">Degree</label>
                        <select
                            id="filterDegreeName"
...
      {filteredStudents.length > 0 ? (
        <div className="overflow-x-auto bg-white rounded-lg shadow-xl overflow-y-auto relative mt-6 border border-gray-100">
          <table className="w-full whitespace-nowrap table-auto">
...
              {filteredStudents.map((student) => (
                <tr key={student._id} className="border-b border-gray-100 hover:bg-indigo-50 transition duration-150 ease-in-out">
                  <td className="py-3 px-4 text-gray-800 font-medium">{student.name}</td>
                  <td className="py-3 px-4 text-gray-600">
                    <p>{student.cnic}</p>
                    <p className='text-xs text-gray-500'>{student.rollNumber && `Roll: ${student.rollNumber}`}</p>
                  </td>
                  <td className="py-3 px-4 text-gray-600">
...
                    <button
                      onClick={() => handleViewStudent(student)}
                      className="text-indigo-600 hover:text-indigo-800 transition-colors duration-200 p-2 rounded-full hover:bg-indigo-100"
                      title="View Student Details"
                    >
                      <EyeIcon className="h-5 w-5" />
                    </button>
