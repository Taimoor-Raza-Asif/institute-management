...
//   }
//   if (expectedReturn < start) {
//       res.status(400);
//       throw new Error('Expected return time must be after the start date.');
//   }
//   // Prevent applying for past leaves if not admin
//   if (requestedByUserRole !== 'admin' && start < now) {
//       res.status(400);
//       throw new Error('Staff cannot apply for past leave dates.');
//   }
...
//     startDate: start,
//     endDate: end,
//     addressGoingTo: addressGoingTo || '', // Optional
//     reason,
//     pickerName: pickerName || '',
//     pickerRelation: pickerRelation || '',
//     pickerPhoneNumber: pickerPhoneNumber || '',
//     pickerCnicNumber: pickerCnicNumber || '',
//     leaveTime: leaveTime ? new Date(leaveTime) : null,
//     expectedReturnTime: expectedReturn,
//     requestedBy: requestedByUserId,
//     status: requestedByUserRole === 'admin' ? 'Approved' : 'Pending', // Admin can auto-approve
//   });
...
//   // If not admin, staff can only see their own leave requests
//   if (userRole !== 'admin') {
//     // Assuming req.user.profileId holds the staff member's _id for non-admin roles
//     if (!req.user.profileId) {
//         res.status(403);
//         throw new Error('User profile not linked for leave request access.');
//     }
//     filter.staff = req.user.profileId;
//   }

//   const staffLeaveRequests = await StaffLeaveRequest.find(filter)
//     .populate('staff', 'name employeeId staffType') // Populate staff details
//     .populate('approvedBy', 'name role') // Populate approver details
//     .populate('requestedBy', 'name role'); // Populate requestor details

//   // Add virtual property for isPastDue
//   const formattedRequests = staffLeaveRequests.map(request => ({
//     ...request.toObject({ virtuals: true }), // Include virtuals
//     isPastDue: request.isPastDue // Ensure this is explicitly included for frontend
//   }));
...
//     .populate('staff', 'name employeeId staffType')
...
//   // Authorization check: Admin can view any, staff can view their own
//   if (userRole !== 'admin' && staffLeaveRequest.staff.toString() !== req.user.profileId.toString()) {
//     res.status(403);
...
// // @desc    Update staff leave request status (Approved/Rejected) and actual return time (for Admin only)
// // @route   PUT /api/staff-leave/:id
// // @access  Private (Admin)
// export const updateStaffLeaveRequest = asyncHandler(async (req, res) => {
//   const { status, actualReturnTime } = req.body;
//   const userRole = req.user.role;
...
//   // Only Admin can update status or actual return time
//   if (userRole !== 'admin') {
//     res.status(403);
//     throw new Error('Not authorized to update staff leave requests');
//   }

//   // Update status (only if status is provided)
//   if (status && ['Approved', 'Rejected'].includes(status)) {
//     staffLeaveRequest.status = status;
//     staffLeaveRequest.approvedBy = approverId;
//     // Add approvedRejectedAt field to schema if needed
//   }

//   // Update actual return time and isReturned status
//   if (actualReturnTime) {
//     staffLeaveRequest.actualReturnTime = new Date(actualReturnTime);
//     staffLeaveRequest.isReturned = true; // Mark as returned
//   } else if (actualReturnTime === null) {
//       // Allow unsetting actualReturnTime (e.g., if entered by mistake)
...
//   } else if (userRole !== 'admin' && staffLeaveRequest.staff.toString() === req.user.profileId.toString() && staffLeaveRequest.status === 'Pending') {
//     await staffLeaveRequest.deleteOne();
