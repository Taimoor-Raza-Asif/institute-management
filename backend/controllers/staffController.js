...
// @desc    Update a teacher's assigned classes (Admin only)
// @route   PUT /api/staff/:id/assign-classes
...
    // Define all valid dynamic types
    const VALID_ACADEMIC_TYPES = ['Class', 'BS', 'Almiya', 'Hifaz'];

    // Validate each entry
    for (const cls of assignClasses) {
        // FIX 1: Recognize all new academic types
        if (!cls.type || !VALID_ACADEMIC_TYPES.includes(cls.type)) {
            return res.status(400).json({ error: `Each class must have a valid type (${VALID_ACADEMIC_TYPES.join(', ')})` });
        }

        // FIX 2: Correctly validate fields based on the new types
        if (['Class', 'Almiya'].includes(cls.type) && !cls.classNumber) {
            return res.status(400).json({ error: `${cls.type} type must have classNumber` });
        }
        if (cls.type === 'BS' && (!cls.degreeName || !cls.semester)) {
            return res.status(400).json({ error: 'BS type must have degreeName and semester' });
        }
        
        // Hifaz type requires no classNumber/degree/semester but still needs subjects defined
        
        if (!Array.isArray(cls.subjects) || cls.subjects.length === 0) {
            return res.status(400).json({ error: 'Each assignment must have at least one subject' });
        }
    }
...
      req.params.id,
      { assignClasses },
      { new: true, runValidators: true } // Added runValidators for Mongoose model validation
    );
