...
import { postFeeToLedger, reverseEntries } from '../utils/ledgerService.js';
...
export const createFeeRecord = async (req, res) => {
  try {
    ...
    // Post to ledger (non-blocking — runs after response sent)
    if (paymentMethod !== 'Auto' && savedFeeRecord.receivedAmount > 0) {
      const feeForLedger = { ...savedFeeRecord.toObject(), studentId: { name: student?.name || '' } };
      postFeeToLedger(feeForLedger).catch(err => console.error('[Ledger] Fee post error:', err.message));
    }
