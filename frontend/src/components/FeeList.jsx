...
//       <h1 className="text-3xl sm:text-4xl font-bold text-center text-indigo-800 mb-8">Fee Management</h1>
...
const FeeList = () => {
  const [fees, setFees] = useState([]);
  const [studentsForForm, setStudentsForForm] = useState([]); // Students for dropdown in FeeForm
  const [editingFee, setEditingFee] = useState(null);
  const [modalOpen, setModalOpen] = useState(false);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  ...
  }, [debouncedSearchTerm, filterMonth, filterYear, filterReceivedBy, filterPaymentMethod, currentUser]);

  const fetchStudentsForDropdown = useCallback(async () => {
    try {
      const response = await api.get('/students'); // Assuming this endpoint is accessible to Admin/Accountant
      if (Array.isArray(response.data)) {
        setStudentsForForm(response.data);
      } else {
        console.error("API response for students dropdown is not an array:", response.data);
        setStudentsForForm([]);
      }
    } catch (err) {
  ...
  }, [fetchFees, fetchStudentsForDropdown, currentUser, canAddFee, canEditFee]); // Added currentUser and permissions to dependencies
  ...
  const handleCloseModal = () => {
    ...
    setEditingFee(null);
    setIsFeeFormViewMode(false);
    fetchFees(); // Refresh list after modal closes
  };

  const currentYear = new Date().getFullYear();
  const yearOptions = Array.from({ length: 11 }, (_, i) => (currentYear - 5 + i).toString()); // From 5 years ago to 5 years from now
  ...
  // Only render if currentUser is available
  if (!currentUser) {
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-6">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-3">
          {/* Search Bar (only for Admin/Accountant) */}
        ...
        {/* Advanced Filters (only for Admin/Accountant) */}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Received Amount</th>
              ...
              fees.map((fee) => (
                <tr key={fee._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {fee.studentId?.cnic || 'N/A'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {fee.month} {fee.year}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    PKR {parseFloat(fee.totalFee).toFixed(2)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    PKR {parseFloat(fee.receivedAmount).toFixed(2)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    PKR {parseFloat(fee.dueAmount).toFixed(2)}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {new Date(fee.receivedDate).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {fee.receivedBy}
                  </td>
      ...
      <FeeModal isOpen={modalOpen} onClose={handleCloseModal}>
        <FeeForm
          ...
          onClose={handleCloseModal}
          isViewMode={isFeeFormViewMode}
          fetchStudents={fetchStudentsForDropdown} // Pass the student fetcher for FeeForm
        />
      </FeeModal>
    </div>
  );
