...
const markAttendance = asyncHandler(async (req, res) => {
  ...
  // Role-based validation (MODIFIED)
  const user = req.user; //
  if (user.role === 'teacher' && type === 'Student') { //
    const teacher = await Staff.findById(user.profileId).select('assignClasses').lean(); //
    if (!teacher) { //
    ...
    // Check if the teacher is authorized to mark for these classes/semesters/Hifaz
    const assignedClasses = teacher.assignClasses || []; //
    const isAuthorized = attendanceRecords.every(record => { //
      // Use generic class field or degree/semester
      return assignedClasses.some(ac => 
        (ac.type === 'Class' && record.studentClass === 'Class' && ac.classNumber === record.studentClassNumber) ||
        (ac.type === 'Almiya' && record.studentClass === 'Almiya' && ac.classNumber === record.studentClassNumber) ||
        (ac.type === 'BS' && record.studentClass === 'BS' && ac.degreeName === record.studentDegreeName && ac.semester === record.studentSemester) ||
        (ac.type === 'Hifaz' && record.studentClass === 'Hifaz')
      ); //
    }); //
        ...
        $set: {
          ...
          ...(type === 'Student' && {
            studentDetails: {
              ...
              currentJuz: record.currentJuz, // NEW FIELD
              currentSurah: record.currentSurah, // NEW FIELD
            }
          }),
        },
      },
...
export const getAssignedStudents = asyncHandler(async (req, res) => {
    try {
        ...
        const assignedClasses = teacher.assignClasses || []; //
        const orConditions = assignedClasses.map(assignment => { //
          // Case 1: Class or Almiya
          if (['Class', 'Almiya'].includes(assignment.type) && assignment.classNumber) {
            return { class: assignment.type, classNumber: assignment.classNumber };
          // Case 2: BS
          } else if (assignment.type === 'BS' && assignment.degreeName && assignment.semester) {
            return { class: 'BS', degreeName: assignment.degreeName, semester: assignment.semester };
          // Case 3: Hifaz (Teacher assigned to Hifaz can see all Hifaz students)
          } else if (assignment.type === 'Hifaz') {
            return { class: 'Hifaz' };
          }
          return null;
        }).filter(Boolean); //
