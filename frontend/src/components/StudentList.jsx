...
const StudentList = () => {
  ...
  }, [fetchStudents, currentUser]); // fetchStudents itself has buildStudentFilterQueryParams as dependency, which covers all filters
  ...
  return (
    <div className="p-6">
      ...
      {/* --- Search and Filter Toggle Section (Hidden for Student Role) --- */}
      { !isStudentRole && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center space-x-3 mb-3">
            <div className="relative flex-grow">
        ...
        <table className="min-w-full table-auto border-separate border-spacing-y-2 border-white shadow-lg rounded-lg overflow-hidden">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="p-2 border border-white w-40">Address</th>
            ...
            {Array.isArray(displayedStudents) && displayedStudents.length > 0 ? (
              displayedStudents.map((s, index) => (
                ...
                >
                  ...
                  <td className="border border-white p-2">{s.cnic || 'N/A'}</td>
                  <td className="border border-white p-2">{s.dob ? new Date(s.dob).toLocaleDateString() : 'N/A'}</td>
                  <td className="border border-white p-2">{s.gender || 'N/A'}</td>
                  <td className="border border-white p-2">{s.email || 'N/A'}</td>
                  <td className="border border-white p-2 w-40 overflow-hidden whitespace-nowrap text-ellipsis" title={s.address}>{s.address || 'N/A'}</td>
                  <td className="border border-white p-2">{s.guardianContact}</td>
                  <td className="border border-white p-2">{s.additionalContact || 'N/A'}</td>
                  <td className="border border-white p-2">{s.admissionDate ? new Date(s.admissionDate).toLocaleDateString() : 'N/A'}</td>
                  <td className="border border-white p-2">
                    {s.class === 'Class' ? s.classNumber || 'N/A' : s.class}
                  </td>
                  <td className="border border-white p-2">
                  ...
                  <td className="border border-white p-2">{s.feePerMonth || 'N/A'}</td>
                  ...
                  <td className={`border border-white p-2 font-semibold
                      ...
                      s.studentStatus === 'Withdrawn' ? 'text-orange-600' :
                        ...
                        s.studentStatus === 'Graduated' ? 'text-purple-600' :
                        'text-gray-600'}
                  `}>
                    {s.studentStatus}
                  </td>
